...
class CouplingLayer(nn.Module):
    def __init__(self,network,mask,in_channels,K,B):
        '''
    ...
    def forward(self,z,log_det,reverse=False):
        ...
        W,H,D  = torch.split(nn_out,selk.K,dim=1)
        '''
        ...
        if not reverse:
            z_out,ld = unconstrained_rqs(z_out,W,H,D,self.B)
            log_det += torch.sum(ld,dim=1)
            out_nr = self.nn(outputs)
            W,H,D  = torch.split(out_nr,selk.K,dim=1)
            W,H = torch.softmax(W,dim=1),torch.softmax(H,dim=1)
            W,H = torch.softmax(W,dim=1),torch.softmax(H,dim=1)
            W,H = (2*self.B) * W, (2*self.B) * H
            D = F.softplus(D)

            z_in, ldz = unconstrained_rqs(z_in,W,H,D,self.B)
            ...
            z_in,ld = unconstrained_rqs(z_in,W,H,D,self.B,inverse=True)
            log_det += torch.sum(ld,dim=1)
            out = nn(z_in)

            W,H,D  = torch.split(out,selk.K,dim=1)
            W,H = torch.softmax(W,dim=1),torch.softmax(H,dim=1)
            W,H = (2*self.B) * W, (2*self.B) * H
            D = F.softplus(D)

            z_out, ld = unconstrained_rqs(z_out,W,H,D,self.B,inverse=True)
            log_det += torch.sum(ld,dim=1)
